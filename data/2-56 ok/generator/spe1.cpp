...
signed main() {
    ...
    int begin = 18;
    int end = 18;
    for(int i = begin; i <= end; ++i) solve(problemID, i, "out");
...
void solve(str problemID, int dataID, str in_out) {
    ...
    println(n, k);
    vec<int> a(30);
    for(int i = 0; i < 10; ++i) a[i] = 30;
    for(int i = 10; i < 20; ++i) a[i] = 60;
    for(int i = 20; i < 30; ++i) a[i] = 90;
    std::shuffle(All(a), r.data());
    for(int i = 0; i < 29; ++i) print(a[i]);
    println(a.back());
