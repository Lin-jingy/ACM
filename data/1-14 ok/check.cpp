...
void solve(str problemID, str dataID, str in_out) {
    str address = std::format("C:/Users/123/Desktop/programming/VScode/ACM/data/{} ok/data/{}.in",problemID,dataID);
    log(dataID);
    ...
    ensure(S >= 1 and S <= (long long)10'000'000'000'000'000);
    for(int i = 1; i <= n; ++i) {
        int a;
        std::cin >> a;
        ensure(a >= 1 and a <= 1'000'000'000);
    }
