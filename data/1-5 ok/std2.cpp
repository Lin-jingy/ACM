...
int main () {
    ...
    std::string address1 = std::format("C:/Users/123/Desktop/programming/VScode/ACM/data/5 ok/data/{}.in", III);
    std::string address2 = std::format("C:/Users/123/Desktop/programming/VScode/ACM/data/5 ok/data/r{}.out", III);
    freopen(address1.data(), "r", stdin);
    freopen(address2.data(), "w", stdout);

    n = 0;
    m = 0;
    for(int i = 0; i < (2 * N + 10) ;++i) tree[i] = {0,0};
    for(int i = 0; i < (2 * N + 10) ;++i) same[i] = 0;
    memset(p, 0, sizeof(p));
    for(int i = 0; i < N; ++i) sa[i] = {0, 0};
    for(int i = 0; i < N; ++i) sb[i] = {0, 0};
