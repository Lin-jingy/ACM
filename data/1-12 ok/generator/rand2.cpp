...
void solve(str problemID, str dataID, str in_out) {
    ...
    int n = 50'000;
    int k = r.get(1, 1'000'000'000);
    println(n, k);
    for(int i = 1; i < n; ++i) print(i % 2 ? std::max(k - r.get(1, k + 100), 1LL) : std::min(k + r.get(1, k + 100), 1'000'000'000LL));
    println(std::max(k - r.get(1, k + 100), 0LL));
    // !end test
