...
void solve(str problemID, str dataID, str in_out) {
    ...
    int N = r.get(1, 100'000);
    int A = r.get(0, 10'000);
    int R = r.get(0, 10'000);
    int M = r.get(0, 10'000);
    std::cout << std::format("{} {} {} {}", N, A, R, M) << std::endl;
    for(int i = 1; i <= N; ++i) {
        if(i != N) std::cout << r.get(0, 1000'000'000) << ' ';
        else std::cout << r.get(0, 1000'000'000) << std::endl;
    }
