...
signed main() {
    ...
    solve(n);
    std::deque<int> mid;
    for(int i = 0; i < n; ++i) if((a[i] ^ i) != a[i] + i) mid.pb(i);
    if(mid.size() % 2 == 1) mid.push_front(0);
    while(!mid.empty()) std::swap(a[mid.front()], a[mid.back()]), mid.pop_back(), mid.pop_front();
    for(int i = 0; i < n; ++i) std::cout << a[i] << ' ';
    for(int i = 0; i < n; ++i) if((a[i] ^ i) != a[i] + i) logs(a[i], i);
    return 0;
}
void solve(int n) {
    if(n <= 0) return ;
