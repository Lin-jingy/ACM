...
#endif
#if __SIZEOF_POINTER__ == 8 && __GNUC__
...
void solve() {
    ...
    vec<std::set<int>> v(n + 1);
    v[1].insert(a[1]);
    vec<int> dp = {0, a[1]};
    for(int i = 2; i <= n; ++i) {
        auto it = std::lower_bound(All(dp), a[i]);
        if(it == dp.end()) dp.pb(a[i]), v[dp.size() - 1].insert(a[i]);
        else *it = a[i], v[it - dp.begin()].insert(a[i]);
    }

    int ans = dp.size() - 1;
    while(v[ans].size() > 1) v[ans].erase(v[ans].begin());

    vec<int> copy;
    int mx = n + 100;
    for(int i = ans; i >= 1; --i) {
        while(*v[i].rbegin() >= mx) v[i].erase(--v[i].end());
        copy.pb(*v[i].rbegin());
        mx = *v[i].rbegin();
    }
    vec<int> num;
    for(int i = 1; i <= n; ++i) {
        if(copy.empty()) break;
        if(a[i] > copy.back()) num.pb(i);
        if(a[i] == copy.back()) copy.pop_back();
    }
    std::cout << ans << ' ' << num.size() << '\n';
    std::cout << num << '\n';
}