...
void solve() {
    ...
    int ans = INF;
    min_heap<std::tuple<int, int, int>> q;
    vvec<bool> vis(n + 1, vec<bool>(n + 1));
    q.emplace(0, s1, s2);
    while (!q.empty()) {
        auto [dis, s1, s2] = q.top();
        q.pop();
        ...
        for (auto i : v1[s1]) {
            for (auto j : v2[s2]) {
                if (s1 == s2 && i == j) {
                    ans = std::min(ans, dis + std::abs(j - i));
                    goto A;
                }
                q.emplace(dis + std::abs(j - i), i, j);
            }
        }
    }
A:;
