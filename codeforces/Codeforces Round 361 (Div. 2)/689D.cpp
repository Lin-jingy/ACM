...
void solve() {
    ...
    vec<int> a(n), b(n);
    std::cin >> a >> b;
    sparseTable<int> A(a, [](int x, int y) { return std::max(x, y); });
    sparseTable<int> B(b, [](int x, int y) { return std::min(x, y); });
    long long ans = 0;
    for (int i = 0; i < n; ++i) {
        int l = i, r = n - 1, kl = i, kr = i - 1;
        while (l <= r) {
            int mid = (l + r) >> 1;
            if (A(i, mid) <= B(i, mid)) l = mid + 1, kr = mid;
            else
                r = mid - 1;
        }
        l = i, r = n - 1;
        while (l <= r) {
            int mid = (l + r) >> 1;
            if (A(i, mid) >= B(i, mid)) r = mid - 1, kl = mid;
            else
                l = mid + 1;
        }
        if (kl >= n or A(i, kl) != B(i, kl)) continue;
        if (kr < 0 or A(i, kr) != B(i, kr)) continue;
        if (kl > kr) std::swap(kl, kr);
        ans += kr - kl + 1;
    }
    std::cout << ans << '\n';
}