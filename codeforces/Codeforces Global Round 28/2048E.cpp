...
std::bitset<2005> bit1;
std::bitset<1005> bit2;
short ans[2005][1005];
int n, m;

void dfs(bool f, int x, int id) {
    if (!f) {
        bit1[x] = 0;
        int pos = bit2._Find_first();
        if (pos < m) ans[x][pos] = id, dfs(!f, pos, id);
    } else {
        bit2[x] = 0;
        int pos = bit1._Find_first();
        if (pos < 2 * n) ans[pos][x] = id, dfs(!f, pos, id);
    }
}
...
void solve() {
    std::cin >> n >> m;
    for (int i = 0; i < n + n; ++i) {
        memset(ans[i], 0, sizeof(short) * (m + 1));
    }
