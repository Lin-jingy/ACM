...
void solve() {
    int a, b, c, d;
    std::cin >> b >> c >> d;
    // vec<int> B = get(b), C = get(c), D = get(d);
    // vec<int> A(63);
    std::bitset<63> B(b), C(c), D(d), A;
    for (int i = 0; i < 63; ++i) {
        std::array<int, 3> tmp = {B[i], C[i], D[i]};
        if (tmp == arr<int, 3>{0, 0, 0}) A[i] = 0;
        else if (tmp == arr<int, 3>{0, 0, 1}) A[i] = 1;
        else if (tmp == arr<int, 3>{0, 1, 0}) A[i] = 0;
        else if (tmp == arr<int, 3>{0, 1, 1}) {
            std::cout << -1 << '\n';
            return;
        } else if (tmp == arr<int, 3>{1, 0, 0}) {
            std::cout << -1 << '\n';
            return;
        } else if (tmp == arr<int, 3>{1, 0, 1}) A[i] = 0;
        else if (tmp == arr<int, 3>{1, 1, 0}) A[i] = 1;
        else if (tmp == arr<int, 3>{1, 1, 1}) A[i] = 0;
    }
