...
void solve() {
    ...
    dfs(dfs, 1, 0);
    sparseTable<int> st(list, [](int a, int b) { return std::max(a, b); });
    w[0] = 0;

    int ans = 0;
    for (int i = 1; i <= n; ++i) {
        int max = 0;
        if (in[i] > 1) max = std::max(max, st(1, in[i] - 1));
        if (out[i] < n) max = std::max(max, st(out[i] + 1, n));
        if (max > w[i] && w[ans] < w[i]) ans = i;
    }
    std::cout << ans << '\n';
}