...
void solve() {
    int n, m;
    std::cin >> n >> m;
    vector<rope<int> *> version(m + 1);
    auto find = [&](auto self, int r, int p) -> int {
        if (version[r]->at(p) < 0) return p;
        else return self(self, r, version[r]->at(p));
    };
    version.front() = new rope<int>;
    for (int i = 0; i <= n; ++i) version.front()->push_back(-1);
    for (int i = 1; i <= m; ++i) {
        int op;
        std::cin >> op;
        if (op == 1) {
            version[i] = new rope<int>(*version[i - 1]);
            int a, b;
            std::cin >> a >> b;
            a = find(find, i, a);
            b = find(find, i, b);
            int sizeA = version[i]->at(a);
            int sizeB = version[i]->at(b);
            if (sizeA <= sizeB) {
                version[i]->replace(b, version[i]->at(b) - version[i]->at(a));
                version[i]->replace(a, b);
            } else {
                version[i]->replace(a, version[i]->at(a) - version[i]->at(b));
                version[i]->replace(b, a);
            }

        } else if (op == 2) {
            int k;
            std::cin >> k;
            version[i] = new rope<int>(*version[k]);
        } else {
            version[i] = new rope<int>(*version[i - 1]);
            int a, b;
            std::cin >> a >> b;
