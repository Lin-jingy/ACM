...
void solve() {
    ...
    std::cin >> l >> r;
    int ans = 0, val = 100;
    for(int i = l; i <= std::min((l / 100000 + 1) * 100000, r); ++i) {
        if(ch(i) < val) ans = i, val = ch(i);
    }
    for(int i = l / 100000 + 1; i < r / 100000; ++i) {
        if(ch(a[i]) < val) ans = a[i], val = ch(a[i]);
    }
    for(int i = std::max(r / 100000 * 100000, l); i <= r; ++i) {
        if(ch(i) < val) ans = i, val = ch(i);
    }
    std::cout << ans <<  '\n';
}
...
void init() {
    for(int i = 0; i <= 1e9; i += 100000) {
        int l = i, r = i + 100000;
        if(l / 100000 % 10 == 5) a[i / 100000] = l;
        else a[i / 100000] = r;
    }
}