...
str a, b;
std::map<str, str> map;
void change(int i,str sk, int add){
	str t=sk;
	char c;
	c = sk[i];
	sk[i] = sk[i+add];
	...
	if(!map.count(sk)){
		q.push(sk);
		char aa = '1' + i / 4, bb = '1' + i % 4, cc = '2' + i / 4, dd = '1' + i % 4;
        if(add == 1) aa = '1' + i / 4, bb = '1' + i % 4, cc = '1' + i / 4, dd = '2' + i % 4;
		map[sk] = map[t] + aa + bb + cc + dd;
	}
	return;
}
void solve() {
	...
	while(!q.empty()){
		auto sk = q.front();
		for(int i=0;i<16;i++){
			if(i < 12)change(i, sk, 4);
			if(i % 4 != 3)change(i, sk, 1);
		}
		if(map.count(b) != 0){
			std::cout << map[b].size() / 4 << '\n';
