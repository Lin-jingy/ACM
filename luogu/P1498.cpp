...
void dfs(int n) {
    ...
    auto ca = a;
    int len = 1LL << (n - 1);
    a.assign(1 << n, vec<char>(len * 4, ' '));
    for (int i = 0; i < len; ++i)
        std::copy(ca[i].begin(), ca[i].end(), a[i].begin() + len);
    for (int i = len; i < len * 2; ++i) {
        std::copy(ca[i - len].begin(), ca[i - len].end(), a[i].begin());
        std::copy(ca[i - len].begin(), ca[i - len].end(),
                  a[i].begin() + len + len);
    }
}
