...
#define double long double
    ...
    auto calc = [&](double x, double y) -> double {
        double res = 0;
        for (int i = 0; i < n; ++i) {
            double dx = v[i][0] - x, dy = v[i][1] - y;
            res += std::sqrt(dx * dx + dy * dy) * v[i][2];
        }
        if (res < dis) dis = res, ax = x, ay = y;
        return res;
    };
    ax /= n, ay /= n, dis = calc(ax, ay);
    double nowx = ax, nowy = ay;
    ...
    auto get = [&]() {
        double t = 9982;
        while (t > 0.001) {
            double nxtx = nowx + t * dist(rnd);
            double nxty = nowy + t * dist(rnd);
            double delta = calc(nxtx, nxty) - calc(nowx, nowy);
            if (std::exp(-delta / t) > dist1(rnd)) nowx = nxtx, nowy = nxty;
            t *= 0.997;
        }
        for (int i = 1; i <= 1000; ++i) {
            double nxtx = ax + t * dist(rnd);
            double nxty = ay + t * dist(rnd);
            calc(nxtx, nxty);
        }
    };
    Timer<0> T;
