...
void solve() {
    ...
    for (int i = 1; i <= q; ++i) {
        ...
        g[x].pb(y);
        g[y].pb(x);
        ans.insert({x, y}), ans.insert({y, x});
    }

    for (int i = 1; i <= n; ++i) {
        for (auto j : g[i])
            for (auto k : f[j]) ans.insert({i, k}), ans.insert({k, i});

        for (auto j : f[i])
            for (auto k : g[j]) ans.insert({i, k}), ans.insert({k, i});
    }
    // logs(ans);
    for (int i = 1; i <= n; ++i) {
        for (auto j : f[i]) {
            if (ans.count({i, j})) ans.erase({i, j});
        }
    }
    std::cout << -((int)ans.size() / 2) + n * (n - 1) / 2 << '\n';
}