...
void solve() {
    std::map<int, int, std::greater<>> mp1, mp2;
    int n;
    std::cin >> n;
    int add = 0, need = 0;
    vec<arr<int, 3>> a(n);
    ...
    while (it1 != mp1.end() and it2 != mp2.end()) {
        int min = std::min(it1->second, it2->second);
        cnt -= min * (it2->first - it1->first);
        if (cnt < 0) {
            std::cout << "NIE\n";
            return;
        }
        it1->second -= min, it2->second -= min;
        if (it1->second == 0) it1 = mp1.erase(it1);
        if (it2->second == 0) it2 = mp2.erase(it2);
    }
