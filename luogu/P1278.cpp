...
std::array<vec<Pii>, 5> v;
int ans;
Timer T;
bool op = 0;
void dfs(int end, int k) {
    if(T.get() > 0.5) {
        op = 1;
        return ;
    }
    ans = std::max(ans, k);
    for(auto &[new_end, val] : v[end]) {
...
void dfs2(int end, int k) {
    ans = std::max(ans, k);
    vec<Pii> mid;
    while(!v[end].empty()) {
        mid.pb(v[end].back());
        v[end].pop_back();
        dfs(mid.back().first, k + mid.back().second);
    }
    while(!mid.empty()) v[end].pb(mid.back()), mid.pop_back();
}
...
void solve() {
    ...
    for(int i = 1; i <= n; ++i) {
        str x;
        std::cin >> x;
        v[ch(x.front())].eb(ch(x.back()), x.size());
    } 
    for(int i = 0; i < 5; ++i) {
        if(!op) dfs(i, 0);
        if(op) dfs2(i, 0);
