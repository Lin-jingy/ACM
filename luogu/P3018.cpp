...
void solve() {
    ...
    dfs1(dfs1, 1);
    // logs(has);
    auto dfs = [&](auto self, int p) -> std::pair<min_heap<int>, int> {
        min_heap<int> heap;
        if (v[p].empty()) {
            ans += c[p] * t[p];
            heap.push(t[p]);
        ...
        for (auto i : v[p]) {
            if (i != max) {
                auto it = self(self, i);
                num += it.second;
                while (!it.first.empty())
                    heap.push(it.first.top()), it.first.pop();
            }
        }
        heap.push(t[p]);
        ans += heap.top() * std::max(0LL, c[p] - num);
        return {heap, std::max(c[p], num)};
    };
    dfs(dfs, 1);
    std::cout << ans << '\n';
}