...
void solve() {
    ...
    auto init = [&]() {
        for(int j = 0; j < 3; ++j) for(int k = 0; k < 3; ++k) dp[2][j][k] = INT_MIN;
    };
    auto DP = [&](){
        for(int i = 3; i <= n; ++i) {
            dp[i][0][1] = dp[i-1][1][0] + a[i];
            dp[i][0][2] = std::max(dp[i-1][2][0], dp[i-1][2][1]) + a[i];
            dp[i][1][0] = std::max(dp[i-1][0][1], dp[i-1][0][2]) + b[i];
            dp[i][1][2] = std::max(dp[i-1][2][1], dp[i-1][2][0]) + b[i];
            dp[i][2][0] = std::max(dp[i-1][0][2], dp[i-1][0][1]) + c[i];
            dp[i][2][1] = dp[i-1][1][2] + c[i];
        }
    };
    int ans = 0;
    init();
    dp[2][1][0] = a[1] + b[2];
    DP();
    for(int i = 0; i < 3; ++i) ans = std::max({ans, dp[n][1][i], dp[n][2][i]});
    ...
    dp[2][0][1] = b[1] + a[2];
    DP();
    for(int i = 0; i < 3; ++i) ans = std::max({ans, dp[n][0][i]});

    init();
    dp[2][2][1] = b[1] + c[2];
    DP();
    for(int i = 0; i < 3; ++i) ans = std::max({ans, dp[n][2][i]});

    init();
    dp[2][0][2] = c[1] + a[2];
    DP();
    for(int i = 0; i < 3; ++i) ans = std::max({ans, dp[n][1][i], dp[n][0][i]});
