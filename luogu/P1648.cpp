...
int getDis(const vec<int> &a, const vec<int> &b) {
    assert(a.size() == b.size());
    int ans = 0;
    for (int i = 0; i < a.size(); ++i) ans += std::abs(a[i] - b[i]);
    return ans;
}
...
void solve() {
    int n, d;
    std::cin >> n >> d;
    vec<vec<int>> a(n, vec<int>(d));
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < d; ++j) std::cin >> a[i][j];
    vec<vec<int>> p(1 << d, vec<int>(d, 0));
    for (int i = 0; i < 1 << d; ++i) {
        std::bitset<4> bit(i);
        for (int j = 0; j < d; ++j) {
            if (bit[j]) p[i][j] = inf;
    ...
    for (int i = 0; i < p.size(); ++i) {
        vec<int> dis;
        for (int j = 0; j < n; ++j) dis.push_back(getDis(p[i], a[j]));
        // std::sort(All(dis));
        ans = std::max(
            ans, *std::max_element(All(dis)) - *std::min_element(All(dis)));
    }
    std::cout << ans << '\n';
}