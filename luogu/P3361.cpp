...
std::pair<int, double> p[25][25];
void solve() {
    int n;
    std::cin >> n;
    int rxa, rxc, rya, ryc, rza, rzc;
    std::cin >> rxa >> rxc >> rya >> ryc >> rza >> rzc;
    int x = 0, y = 0, z = 0;
    for (int i = 0; i < n; ++i) {
        double xx, yy;
        x = (y * rxa + rxc) % rp;
        y = (z * rya + ryc) % rp;
        z = (x * rza + rzc) % rp;
        xx = x % 20 + (y % 10) * 0.1 + (z % 10) * 0.01;
        x = (y * rxa + rxc) % rp;
        y = (z * rya + ryc) % rp;
        z = (x * rza + rzc) % rp;
        yy = x % 20 + (y % 10) * 0.1 + (z % 10) * 0.01;
        p[(int)xx][(int)yy].first++;
        p[(int)xx][(int)yy].second += xx + yy;
    }
    double ans = 0;
    int num = 0;
    auto get = [&](int x, int y) -> void {
        for (int i = 0; i < x; ++i) {
            for (int j = 0; j < y; ++j) {
                ans += p[i][j].first * p[x][y].second;
                ans -= p[x][y].first * p[i][j].second;
                num += p[i][j].first * p[x][y].first;
            }
        }
    };
    for (int i = 0; i < 20; ++i) {
