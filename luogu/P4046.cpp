...
void solve() {
    ...
    std::cin >> m;
    for (int i = 1; i <= m; ++i)
        for (int j = 1; j <= m; ++j) std::cin >> dis[i][j];
    vec<int> list;
    int x;
    while (std::cin >> x) list.pb(x);
    int n = list.size();
    list.insert(list.begin(), 3);
    // logs(list);

    vvec<int> dp(m + 10, vec<int>(m + 10, inf));
    auto last = dp;
    dp[1][2] = 0;

    for (int i = 1; i <= n; ++i) {
        for (auto &_last : last) std::ranges::fill(_last, inf);
        std::swap(dp, last);
        for (int j = 1; j <= m; ++j) {
            for (int k = 1; k <= m; ++k) {
                if (last[j][k] == inf) continue;
                dp[j][k] =
                    std::min(last[j][k] + dis[list[i - 1]][list[i]], dp[j][k]);
                dp[list[i - 1]][k] =
                    std::min(last[j][k] + dis[j][list[i]], dp[list[i - 1]][k]);
                dp[j][list[i - 1]] =
                    std::min(last[j][k] + dis[k][list[i]], dp[j][list[i - 1]]);
            }
        }
    }
    int ans = inf;
    for (const auto &i : dp) ans = std::min(ans, *std::min_element(All(i)));
    std::cout << ans << '\n';
}