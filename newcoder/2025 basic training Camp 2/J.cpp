...
void solve() {
    ...
    uset<std::string> a, b, c;

    for (int i = 1; i <= n; ++i) {
        std::string id, date, time;
        std::cin >> id >> date >> time;
        if (std::stoi(date.substr(0, 4)) != H ||
            std::stoi(date.substr(5, 2)) != M)
            continue;

        int h = std::stoi(time.substr(0, 2)) * 10000 +
                std::stoi(time.substr(3, 2)) * 100 +
                std::stoi(time.substr(6, 2));
        if (h >= 70000 && h <= 90000 || h >= 180000 && h <= 200000)
            a.insert(id);
        if (h >= 110000 && h <= 130000) b.insert(id);
        if (h >= 220000 || h <= 10000) c.insert(id);
    }
