...
void solve() {
    std::cin >> n;
    vvec<int> p(3, vec<int>(10));

    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < n; ++j) {
            std::cin >> a[i][j];
            if (a[i][j] != '?') p[j % 3][a[i][j] - '0']++;
        }
    }

    vec<vec<int>> aviliable(3, vec<int>(10));

    {
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 1; j <= 9; ++j) {
                if (p[i][j] != 0) {
                    if (s.count(j)) RETURN(std::cout << 0 << '\n');
                    s.insert(j);
                }
            }
        }
    ...
    int pla[9];
    std::iota(pla, pla + 9, 1);

    Mint tot = 0;
    Mint aaa = ans[0] * ans[1] * ans[2];
    do {
        int f = 0;
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                if (a[i][j] != '?' && a[i][j] - '0' != pla[f]) goto NEXT;
                if (a[i][j] == '?' && !aviliable[j][pla[f]]) goto NEXT;
                ++f;
            }
        }
        tot += aaa;
    ...
    NEXT:;
    } while (std::next_permutation(pla, pla + 9));
