...
void solve() {
    int n, k;
    std::cin >> n >> k;
    int min = std::max(n - 26, 0LL);
    int max = n - 1;
    if (k > max || k < min) RETURN(std::cout << "NO\n");

    std::cout << "YES\n";

    auto repeat = [](int n) -> std::string {
        std::string s;
        while (n--) s += 'a';
        return s;
    };

    auto range = [](int n) -> std::string {
        std::string s;
    ...
    std::string s;
    if (k * 2 >= n)
        std::cout << range(n - k) << repeat(2 * k - n) << range(n - k) << '\n';
    else std::cout << range(n - k) << range(k) << '\n';
