...
void solve() {
    ...
    auto change = [&](auto &ca, int i, int j) -> void {
        ca[i][j] = 1 ^ ca[i][j];
        if (i + 1 <= n) ca[i + 1][j] = 1 ^ ca[i + 1][j];
        if (i - 1 >= 1) ca[i - 1][j] = 1 ^ ca[i - 1][j];
        if (j + 1 <= m) ca[i][j + 1] = 1 ^ ca[i][j + 1];
        if (j - 1 >= 1) ca[i][j - 1] = 1 ^ ca[i][j - 1];
    };

    int ans = inf;
    auto getAns = [&](const std::bitset<10> bit, int x) -> void {
        int num = bit.count();
        auto ca = a;
        rep(i, 1, m) {
            if (bit[i - 1]) change(ca, 1, i);
        }

        rep(i, 1, n - 1) {
            rep(j, 1, m) {
                if (ca[i][j] == x) continue;
                change(ca, i + 1, j);
                ++num;
            }
        }
