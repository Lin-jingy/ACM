...
signed main() {
    ...
    while (T--) {
        ...
        // for (auto [x, y, z] : ans) {
        //     int a1 = std::gcd(x, y) == 1;
        //     int a2 = std::gcd(y, z) == 1;
        //     int a3 = std::gcd(x, z) == 1;
        //     // logs(x, y, z);
        //     assert(a1 + a2 + a3 == 2);
        // }
    }
...
bool solve(int n) {
    ...
    int p = n % 12;
    for (int i = 0; i <= n - 12 - p; i += 12) {
        ans.eb(i + 1, i + 2, i + 4);
        ans.eb(i + 6, i + 7, i + 8);
        ans.eb(i + 10, i + 11, i + 12);
        ans.eb(i + 3, i + 5, i + 9);
    }
    ...
    switch (p) {
        case 3:
            ans.eb(i / 2 + 1, i + 2, i + 3);
            for (auto &[x, y, z] : ans) {
                if (x == i / 2 + 1) x = i + 1;
                else if (y == i / 2 + 1) y = i + 1;
