...
Mint get(int i) {
    if (i <= 0) return 0;
    return Mint(i * Mint(i + 1) * Mint(i * 2 + 1) / Mint(12)) +
           Mint(i * Mint(i + 1)) / Mint(4);
}

Mint get1(int l, int r) {
    if (l > r) std::swap(l, r);
    Mint n = r - l + 1;
    return Mint(n * Mint(l + r)) / Mint(2);
}

Mint get2(int l, int r, int n) {
    if (l > r) std::swap(l, r);
    return (Mint(n) * Mint(l + r)) / Mint(2);
}
...
void solve() {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        tmp += get(r - l1) - get(l - l1 - 1);
        tmp += get(r1 - l) - get(r1 - r - 1);

    A:;

        auto fact = [&](int ll, int rr) -> void {
            if (ll > rr) return;
            int len = rr - ll + 1;
            Mint mn = get1(std::abs(ll - l1), std::abs(ll - r1));
            Mint mx = get1(std::abs(rr - l1), std::abs(rr - r1));
            tmp += get2(mn.val(), mx.val(), rr - ll + 1);
        };
        ...
        ans += tmp * cnt / Mint(r2 - l2 + 1) / Mint(r1 - l1 + 1);
    }
    std::cout << ans / cnt << '\n';
}