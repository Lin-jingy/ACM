...
void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        std::bitset<31> bit(a[i]);
        for (int j = 0; j < 31; ++j) {
            pre1[i][j][0] = pre1[i - 1][j][0];
            pre1[i][j][1] = pre1[i - 1][j][1];
        }
        for (int j = 0; j < 31; ++j) pre1[i][j][bit[j]]++;
    }

    memset(pre2[n + 1], 0, sizeof(int) * 31 * 2);

    for (int i = n; i >= 1; --i) {
        std::bitset<31> bit(b[i]);
        for (int j = 0; j < 31; ++j) {
            pre2[i][j][0] = pre2[i + 1][j][0];
            pre2[i][j][1] = pre2[i + 1][j][1];
        }
        for (int j = 0; j < 31; ++j) pre2[i][j][bit[j]]++;
    }
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        x[i] = x[i - 1];
        for (int j = 0; j < 31; ++j) {
            x[i] += Mint(pre2[i][j][!bit[j]]) * Mint(1LL << j);
        }
    }
    ...
    while (q--) {
        ...
        for (int i = 0; i < 31; ++i) {
            int p1[2], p2[2];
            p1[0] = pre1[r][i][0] - pre1[l - 1][i][0];
            p1[1] = pre1[r][i][1] - pre1[l - 1][i][1];
            p2[0] = pre2[r + 1][i][0];
            p2[1] = pre2[r + 1][i][1];

            ans -= Mint(p1[1]) * Mint(p2[0]) * Mint(1LL << i);
            ans -= Mint(p1[0]) * Mint(p2[1]) * Mint(1LL << i);
        }
