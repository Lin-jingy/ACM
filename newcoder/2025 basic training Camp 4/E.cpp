...
void solve() {
    ...
    int ans = 0;
    vvec<int> p1(n + 5, vec<int>(m + 5));
    vvec<int> p2(n + 5, vec<int>(m + 5));
    vvec<int> p3(n + 5, vec<int>(m + 5));
    vvec<int> p4(n + 5, vec<int>(m + 5));
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            p1[i][j] = a[i][j] + p1[i - 1][j - 1];
            p2[i][j] = a[i][j] + p2[i - 1][j + 1];
        }
    }

    for (int i = n; i >= 1; --i) {
        for (int j = m; j >= 1; --j) {
            p3[i][j] = a[i][j] + p3[i + 1][j - 1];
            p4[i][j] = a[i][j] + p4[i + 1][j + 1];
        }
    }
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            ans = std::max(
                ans, p1[i][j] + p2[i][j] + p3[i][j] + p4[i][j] - 3 * a[i][j]);
        }
    }
