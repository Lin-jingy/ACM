...
void solve() {
    int n;
    std::cin >> n;
    std::map<int, vec<int>> mp;
    mp[0].push_back(0);
    vec<int> a(n + 1), xorsum(n + 1);
    for (int i = 1; i <= n; i++) {
        std::cin >> a[i];
        xorsum[i] = xorsum[i - 1] ^ a[i];
        mp[xorsum[i]].push_back(i);
    }
    ...
    for (int i = 1; i <= n; i++) {
        int r = i - 1;
        while (r >= 1) {
            int midl = 1, midr = r, gcd = st(r, i);
            int mnl = r;
            while (midl <= midr) {
                int mid = (midl + midr) >> 1;
                if (st(mid, i) == gcd) mnl = mid, midr = mid - 1;
                else midl = mid + 1;
            }
            auto it = mp.find(gcd ^ xorsum[i]);
            if (it != mp.end())
                ans += upper_bound(All(it->second), r - 1) -
                       lower_bound(All(it->second), mnl - 1);
            r = mnl - 1;
        }
    }
    std::cout << ans << "\n";
