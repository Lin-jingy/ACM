...
void solve() {
    ...
    int mmin = *std::min_element(a.begin() + 1, a.end());
    int mmax = *std::max_element(a.begin() + 1, a.end());

    ans = mmax * 2 - mmin * 2;

    if (std::count(a.begin() + 1, a.end(), mmin) > 1) {
        ans = std::min(ans, mmax - mmin);
    }
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        if (l != 1) {
            mn = std::min(mn, min(1, l - 1));
            mx = std::max(mx, max(1, l - 1));
        }
        if (r != n) {
            mn = std::min(mn, min(r + 1, n));
            mx = std::max(mx, max(r + 1, n));
        }
