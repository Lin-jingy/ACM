...
void solve() {
    ...
    vec<int> ls(rn), rs(rn);
    for (int i = 0; i < rn; ++i) ls[i] = a[i];
    for (int i = 0; i < rn; ++i) rs[i] = a[i + rn];

    int suml = std::accumulate(All(ls), 0LL);
    int sumr = std::accumulate(All(rs), 0LL);

    vec<int> pre1(rn), pre2(rn);
    pre1[0] = ls[0];
    pre2[0] = rs[0];
    for (int i = 1; i < rn; ++i) pre1[i] = pre1[i - 1] + ls[i];
    for (int i = 1; i < rn; ++i) pre2[i] = pre2[i - 1] + rs[i];

    auto calc = [&](const vec<int> &pre, const vec<int> &s, int val) -> int {
        int sum = 0;

        int t = std::upper_bound(All(s), val) - s.begin();
        int num = t;
        int tot = 0;
        if (t - 1 >= 0) tot = pre[t - 1];

        sum += std::abs(num * val - tot);
        num = rn - num;
        tot = pre[rn - 1] - tot;
        sum += std::abs(num * val - tot);
        return sum;
    ...
    int lans = INF, lval, rans = INF, rval;
    for (int i = 0; i < rn; ++i) {
        int val = calc(pre1, ls, ls[i]);
        if (val < lans) lans = val, lval = ls[i];
    }
    for (int i = 0; i < rn; ++i) {
        int val = calc(pre2, rs, rs[i]);
        if (val < rans) rans = val, rval = rs[i];
    }
    // logs(lans, rans);

    if (lval != rval) RETURN(std::cout << lans + rans << '\n');

    int ans = INF;
    ans = std::min(ans, calc(pre1, ls, lval + 1) + rans);
    ans = std::min(ans, calc(pre1, ls, lval - 1) + rans);
    ans = std::min(ans, calc(pre2, rs, rval + 1) + lans);
    ans = std::min(ans, calc(pre2, rs, rval - 1) + lans);
    if (a.front() == a.back()) ans = n / 2;
    std::cout << ans << '\n';
}