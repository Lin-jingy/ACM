...
void solve() {
    ...
    for (int i = 1; i <= n / 2; ++i) {
        for (int j = 1; j <= n / 2; ++j) {
            ...
            if (y > j) {
                while (x != i) {
                    if (x > i) move(x - 1, y, x, y), --x;
                    else move(x + 1, y, x, y), ++x;
                }
                while (y != j) {
                    if (y > j) move(x, y - 1, x, y), --y;
                    else move(x, y + 1, x, y), ++y;
                }
            } else {
                while (y != j) {
                    if (y > j) move(x, y - 1, x, y), --y;
    ...
    std::cout << ans.size() << '\n';
    for (const auto [x1, y1, x2, y2] : ans) {
        std::cout << std::format("{} {} {} {}\n", x1, y1, x2, y2);
    }
}