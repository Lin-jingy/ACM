...
void solve() {
    ...
    while (l <= n) {
        ...
        int len = r - l + 1;
        int d = 0;
        if (len != 1) d = (end - begin) / (len - 1);

        v.push_back({d, begin, len, l, r});
        l = r + 1;
    }

    // for (auto [d, first, num, l, r] : v) {
    //     logs(d, first, num, l, r);
    // }
    ...
    auto check = [&](int X) -> bool {
        ...
        for (auto [d, first, num, l, r] : v) {
            ...
            while (l <= r) {
                int mid = (l + r) >> 1;
                if (first + (mid - 1) * d >= X) l = mid + 1, k = mid;
                else r = mid - 1;
            }
            cnt += k;
        }
    ...
    while (l <= r) {
        int mid = (l + r) >> 1;
        if (check(mid)) l = mid + 1, number = mid;
        else r = mid - 1;
    }
    ...
    for (auto [d, first, num, l, r] : v) {
        ...
        while (l <= r) {
            int mid = (l + r) >> 1;
            if (first + (mid - 1) * d >= number) l = mid + 1, k = mid;
            else r = mid - 1;
        }
        cnt += k;
        tot += k * (first + first + (k - 1) * d) / 2;
    }
