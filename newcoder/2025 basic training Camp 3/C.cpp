...
void solve() {
    ...
    int max = 0;
    for (int i = 0; i < n; ++i) max = std::max(max, (int)s[i].size());

    int ans = s.front().size();
    s.pb("");
    for (int i = 1; i < s.size(); ++i) {
        const auto &last = s[i - 1];
        const auto &now = s[i];

        int same = 0;
        for (int j = 0; j < last.size() && j < now.size(); ++j) {
            if (last[j] == now[j]) ++same;
            else break;
        }

        ans += last.size() - same + now.size() - same;
    }
