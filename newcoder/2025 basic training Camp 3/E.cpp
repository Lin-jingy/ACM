...
void solve() {
    ...
    vec<Pii> v(n);
    std::cin >> v;
    for (auto &i : v) i.first *= 2;
    std::ranges::sort(v);

    vec<std::tuple<int, int, int>> a, b;
    for (int i = 0; i < n; ++i) {
        if (v[i].second == -1) a.eb(v[i].first, -1, i);
        else b.eb(v[i].first, 1, i);
    }
    ...
    auto check = [&](int mid) -> bool {
        auto nowa = a;
        auto nowb = b;
        for (auto &[pos, x, val] : nowa) pos -= mid;
        for (auto &[pos, x, val] : nowb) pos += mid;

        vec<std::tuple<int, int, int>> tot(n);
        std::merge(All(nowa), All(nowb), tot.begin());
        int num = 0;
        for (int i = 0; i < n; ++i) {
            num += std::abs(std::get<2>(tot[i]) - i);
        }
        return num >= k * 2;
    };
