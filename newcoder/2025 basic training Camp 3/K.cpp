...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        std::cin >> num;
        a[i].resize(num);
        for (int j = 0; j < num; ++j) std::cin >> a[i][j];
        vec<int> ra(num + 1);
        for (int j = 1; j <= num; ++j) ra[j] = a[i][j - 1];
        ans += Reverse_pair(ra);
    }

    std::ranges::sort(a, [](const vec<int> &a, const vec<int> &b) {
    ...
    if (ans == k) std::cout << "Yes\n";
    else RETURN(std::cout << "No\n");
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < a[i].size(); ++j) {
            std::cout << a[i][j] << ' ';
            if (i == pi && j == pj - 1) std::cout << val << ' ';
        }
    }
