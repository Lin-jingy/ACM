...
void solve() {
    ...
    auto dfs = [&](auto self, int u, int fa) -> void {
        ...
        for (auto [v, id] : e[u]) {
            ...
            f[u] += f[v];
            f[u] +=
                c[u][1] * (sm[v][0] + c[v][0]) + c[v][1] * (sm[u][0] + c[u][0]);
            f[u] += c[v][0] * sm[u][1] + c[u][0] * sm[v][1];
            for (int j = 0; j < 2; j++) {
                sm[u][j] += sm[v][j] + c[v][j];
                c[u][j] += c[v][j];
            }
    ...
    auto dfs1 = [&](auto self, int u, int fa) -> void {
        for (auto [v, id] : e[u]) {
            ...
            ans2[id] = f[1] - ans[id];
            ans2[id] -=
                cf[1] * (sm[v][0] + c[v][0]) + c[v][1] * (csm[0] + cf[0]);
            ans2[id] -= c[v][0] * csm[1] + cf[0] * sm[v][1];
            if (edg[id].first == u) std::swap(ans[id], ans2[id]);
    ...
    dfs1(dfs1, 1, 0);
    for (int i = 1; i < n; i++) std::cout << ans[i] << ' ' << ans2[i] << "\n";
}